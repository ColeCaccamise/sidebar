...
export default function Pricing({
  ...
  useEffect(() => {
    ...
    async function fetchPlans() {
      await api
        .get(`/teams/${teamSlug}/billing/plans`, {
  ...
  useEffect(() => {
    async function fetchSubscription() {
      await api
        .get(`/teams/${teamSlug}/billing/subscription`, {
          withCredentials: true,
        })
        .then((res) => {
  ...
  async function handleSelectPlan(priceLookupKey: string) {
    ...
    await api
      .post(
        `/teams/${teamSlug}/billing/checkout`,
        {
  ...
  async function handleUpdatePlan(priceLookupKey: string) {
    await api
      .post(
        `/teams/${teamSlug}/billing/subscription/update`,
        {
          price_lookup_key: priceLookupKey,
        },
        {
              ...
              .map(
                (plan: {
                  ...
                  <PricingBox
                    ...
                    subscribeToSuffix={
                      variant === 'onboarding' ? 'for 14 days' : ''
                    }
                  />
                ),
              )}
